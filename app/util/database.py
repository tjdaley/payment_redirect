...
class Database(object):
    ...
    def get_contacts(self, email: str, where: dict = {}, page_num: int = 1, page_size: int = 25) -> list:
        """
        ...
        contacts = self.dbconn[CONTACTS_TABLE].find(where).sort(order_by).skip(skips).limit(page_size)
    ...
    def search_contacts(self, email: str, query: str, page_num: int = 1, page_size: int = 25) -> list:
        """
    ...
    def save_contact(self, email: str, doc: dict) -> dict:
        """
        ...
        if doc['_id'] == '0':
            ...
            result = self.dbconn[CONTACTS_TABLE].insert_one(doc)
        ...
        del doc['_id']
        result = self.dbconn[CONTACTS_TABLE].update_one(filter_, {'$set': doc})
        if result.modified_count == 1:
            message = f"{contact_name}'s record updated"
            return {'success': True, 'message': message}

        message = f"{contact_name}'s record did not update ({result.modified_count})"
        return {'success': False, 'message': message}
